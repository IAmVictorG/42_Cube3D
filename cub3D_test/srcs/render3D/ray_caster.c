...
int compare_vectors(t_vec vector0, t_vec vector1)
{
    ...
    r = (int) vector0.x == (int) vector1.x && (int) vector0.y == (int) vector1.y && (int) vector0.z == (int) vector1.z;
...
void trace_ray(t_general *general) 
{
    ...
    //float d = (direction.y / direction.x);
    float player_angle;
    ...
    // else
    // {
    //     player_angle = 2 * M_PI - player_angle;
    // }
    ...
    for (float angle = fov_start; angle <= fov_end; angle += angle_step)
    {
        ...
        result = find_point_on_screen(general, (t_coord){position.x, position.y, 0}, (t_coord){end_point.x, end_point.y, 0});
        ...
        if(compare_vectors(result.v3, temp) == 0)
        {
        ...
        else
        {
            ...
            //printVec(result.v3);
            //printf("dist = %f wall_height = %d px\n\n", dist, wall_height);
