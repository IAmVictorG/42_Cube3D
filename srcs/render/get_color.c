...
int get_color_wall_east(t_general *general, t_vec ray, int h_wall, int max_wall_h)
{
    ...
    size_wall = general->scene->map.size_wall;
    sprites = general->sprites;
    pixel = sprites->wall_south.data_spr.addr + (((y % size_wall)*general->sprites->wall_south.sprite_w/size_wall) * sprites->wall_south.data_spr.line_length + (h_wall * general->sprites->wall_south.sprite_h/ max_wall_h) * (sprites->wall_south.data_spr.bits_per_pixel / 8));
    color = *(unsigned int *)pixel;
    return (color);
}
