...
void init_window(t_mlib *mlib, t_scene *scene, t_sprites *sprites)
{
    ...
    mlx_hook(mlib->utils.win, 2, 1L<<0, key_press_exit, general);  // Hook pour les touches pressées
    mlx_hook(mlib->utils.win, 3, 1L<<1, key_release, general);
    mlx_hook(mlib->utils.win, 4, 1L<<8, mouse_press, mlib);  // Hook pour les clics de souris
    mlx_hook(mlib->utils.win, 17, 1L<<0, close_window, mlib);  // Hook pour la fermeture de la fenêtre
